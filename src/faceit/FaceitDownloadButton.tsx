...
export default function FaceitDownloadButton() {
  const { component: Captcha, getToken } = useTurnstileCaptcha("download");
  ...
  return (
    <>
      {loading ? (
        <div className="text-leetify flex h-8 w-full items-center justify-center rounded border-0 bg-[#161616]">
          <svg
            aria-hidden="true"
            className="fill-leetify h-5 w-5 animate-spin text-gray-200/60"
            viewBox="0 0 100 101"
            fill="none"
        ...
        <button
          className="text-leetify block h-8 w-full rounded border-0 bg-[#161616] px-6 py-2 font-bold brightness-100 transition-all duration-100 hover:brightness-125"
          onClick={handleClick}
        >
          DOWNLOAD DEMO
        </button>
